...
public class Main {
		...
		while(!que.isEmpty()) {
			...
			for(int i = 0; i < 4; i++) {
				...
				if(checkRange(gx, gy) && map[gx][gy] == 0) {	// 방문 배열 따로 처리 안해도 되는 이유 -> 어차피 que에는 중복으로 안 들어가니까
					edge_que.add(new Point(point[0], point[1], 0, idx));	//현재 좌표, 초기 거리, 현재 육지 번호
					break;
				}
			}
		...
		while(!edge_que.isEmpty()) {
			...
			for(int i = 0; i < 4; i++) {
				...
				if(!visited[tx][ty] && map[tx][ty] == 0) {	// 방문 전+ 바다
					...
					edge_que.add(new Point(tx, ty, 0, 0));
	...
	// dfs 실패
//	static void dfs(int x, int y, int len, int area_idx, boolean [][] visited) {
//		if(map[x][y] == area_idx) return;	// 같은 구역은 더이상 진행 X 	-> 근데 초기에 이러면, 다 실패
//		
//		if(map[x][y] != 0 && map[x][y] != area_idx) {	// 타 육지 도착
...
//			int ty = y + dy[i];
//			
//			if(checkRange(tx, ty) && !visited[tx][ty]) {	// 범위 내  + 아직 방문 전
//				visited[tx][ty] = true;
//				dfs(tx, ty, len+1, area_idx, visited);
//				visited[tx][ty] = false;
//			}
//		}
//	}
